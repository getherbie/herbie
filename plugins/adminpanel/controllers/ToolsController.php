...
class ToolsController extends Controller
{
    ...
    public function deleteCacheAction(ServerRequestInterface $request)
    {
    ...
    public function reformatFileAction(ServerRequestInterface $request)
    {
        $name = $request->getPost('name');
        ...
        foreach ($tempDirs as $td) {
            ...
            if (!empty($path) && is_dir($path)) {
                $dirs[$key] = [
                    'label' => $label,
                    'path' => $path,
                    'count' => 0, //FilesystemHelper::rcount($path)
                ];
            }
        }
        return $dirs;
