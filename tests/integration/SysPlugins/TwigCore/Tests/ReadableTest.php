...
final class ReadableTest extends \Codeception\Test\Unit
{
    ...
    {
        $twig = <<<TWIG
            {%- if '' is fileReadable -%}readable{% else %}not readable{%- endif -%}
        TWIG;
    ...
    {
        $twig = <<<TWIG
            {%- if '@media/dummy.pdf' is fileReadable -%}is readable{% else %}not readable{%- endif -%}
        TWIG;
    ...
    {
        $twig = <<<TWIG
            {%- if '@media/dummy-no-rights.pdf' is fileReadable -%}is readable{% else %}not readable{%- endif -%}
        TWIG;
        $this->assertSame('not readable', $this->twig()->renderString($twig));
    }
    ...
    {
        $twig = <<<TWIG
            {%- if '@media/dummy-xyz.pdf' is fileReadable -%}is readable{% else %}not readable{%- endif -%}
        TWIG;
        $this->assertSame('not readable', $this->twig()->renderString($twig));
    }
}
